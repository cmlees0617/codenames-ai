...
3. **Benchmark boards** — `clue-eval` generates stratified Codenames layouts with GloVe difficulty (stable on `main`)
4. **Predefined clue tests** — `clue-eval` catalog on `feature/spymaster-test-pipeline`
5. **Spymaster test pipeline** — full-game benchmark vs fixed operatives (WIP on that branch)
6. **CNO players** — async loops that talk to codenames.game
7. **CLI** — human-facing argument and prompt layer
8. **Optional example ML** — `cluegen`

!!! warning "Test pipeline in progress"
    Only the full-game benchmark harness is experimental and likely to be massively reworked. Board generation is stable; the catalog is branch-only until merge.
...
| `clue-eval` | `game-core`, `numpy`, optional ML extras | `cluegen`, `cno-sdk`, `cno-bots` |
| `cluegen` | `game-core` | `clue-eval`, `cno-sdk`, `cno-bots` |
...
| Benchmark board sets | Reproducible stratified layouts | [Add benchmark boards & catalog tests](../contributor-guides/adding-clue-test.md) |
