...
**Tests:** use `ScriptedClueAlgorithm` as a template, or unit-test `rank_clues` with a minimal `SpymasterView` (see `packages/cluegen/tests/test_algorithms.py`). Load fixed layouts from [clue-eval](../implementations/clue-eval.md) when you need benchmark boards with difficulty metadata.
